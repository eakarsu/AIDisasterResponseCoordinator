...
/**
 * Gap/CFS Feature Page: `supplyRoutes` lacks `/optimize-supply-distribution`.
 * Slug: supplyroutes-lacks-optimize-supply-distribution
 * Backend: /api/gap-supplyroutes-lacks-optimize-supply-distribution
 */
export default function SupplyroutesLacksOptimizeSupplyDistributionPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-supplyroutes-lacks-optimize-supply-distribution', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>`supplyRoutes` lacks `/optimize-supply-distribution`.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>`supplyRoutes` lacks `/optimize-supply-distribution`.</p>
