...
/**
 * Gap/CFS Feature Page: `shelterRoutes` lacks `/optimize-shelter-assignments`.
 * Slug: shelterroutes-lacks-optimize-shelter-assignments
 * Backend: /api/gap-shelterroutes-lacks-optimize-shelter-assignments
 */
export default function ShelterroutesLacksOptimizeShelterAssignmentsPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-shelterroutes-lacks-optimize-shelter-assignments', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>`shelterRoutes` lacks `/optimize-shelter-assignments`.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>`shelterRoutes` lacks `/optimize-shelter-assignments`.</p>
