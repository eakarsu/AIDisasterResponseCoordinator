function validateRuntime(env = process.env) {
  ...
  if (env.NODE_ENV === 'production' && (env.CORS_ORIGINS || '').includes('*')) {
    errors.push('Wildcard CORS is forbidden in production');
  }
  if (env.NODE_ENV === 'production' && env.ENABLE_LEGACY_EMERGENCY_SURFACES === 'true') {
    errors.push('Legacy emergency/model surfaces cannot be enabled in production');
  }
